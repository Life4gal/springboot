...
const authModal = new Vue({
    el: "#authAddedModal", methods: {
            ...
            if (isEmpty(file)) {
                ...
                $.get("/file/basic/all", {
                    ...
                    if (json.length < 1) {
                        ...
                        const ele = $("#auth-file-list-group");
                        $(ele).empty();
                        $.each(json, function (i, n) {
                            const li = "<li class='list-group-item list-group-item-success'><a href='javascript:' class='auth-file' onclick='$(event.srcElement).parent().remove();' data-key='" + n.id + "'>" + n.localUrl + "</a></li>";
                            $(ele).append(li);
                        });
        ...
        "searchUserInAuth": function () {
            const user = $("#auth-user-search").val();
            if (isEmpty(user)) {
                alerts("内容不能为空");
            } else {
                layer.load(1);
                $.get("/user/all", {
                    ...
                    if (json.length < 1) {
                        ...
                        const ele = $("#auth-user-list-group");
                        $(ele).empty();
                        $.each(json, function (i, n) {
...
function deleteCategory() {
    const srcTr = $(event.srcElement).parents("tr");
    const categoryId = $(srcTr).children("td.category-id").text();
    $.ajax({
...
function showFileShareModal() {
    ...
    const modal = $("#fileAddedModal");
    const ele = $(modal).find(".modal-content");
    if (isMobile()) {
...
function setUsername(id, auth) {
    checkRowIndex();
    const user = app.users[$(selectedRows[rowIndex]).children(".user-index").attr("data-key")];
    const parent = $(id);
...
function setUserFileAuth() {
    checkRowIndex();
    const user = app.users[$(selectedRows[rowIndex]).children(".user-index").attr("data-key")];
    const parent = $("#userFileAuthModal");
    $(parent).find(".username").val(user.username);
...
function setAuth() {
    checkRowIndex();
    const auth = app.auths[$(selectedRows[rowIndex]).children(".auth-index").attr("data-key")];
...
function setModifyFile() {
    checkRowIndex();
    const file = app.files[$(selectedRows[rowIndex]).children(".file-index").attr("data-key")];
    /** @namespace file.localUrl */
...
function getServerFileByPath(addTo) {
    ...
    $.get("/file/server", {path: path}, function (data) {
        ...
        const ele = $("#server-file-list-group");
        $(ele).empty();
        $.each(app.serverFiles, function (i, json) {
            const li = "<li class='list-group-item list-group-item-" + (json.isDirectory ? "primary" : "info") + "' data-key='" + i + "'><a href='javascript:' class='server-file' title='" + json.absolutePath + "' data-toggle='tooltip' onclick='selectServerFile();'>" + json.name + "</a></li>";
            $(ele).append(li);
...
function toggleRowSelectedStatus(ele) {
    if (event.srcElement.toString() === "[object HTMLTableCellElement]") {
        const cb = $(ele).find("input[type='checkbox']")[0];
...
function addToSelectedServerFile(json) {
    ...
    const li = "<li class='list-group-item list-group-item-success' data-key='" + (app.selectedServerFiles.length - 1) + "'><a href='javascript:' class='selected-server-file' title='" + json.absolutePath + "' data-toggle='tooltip' onclick='removeSelectedServerFile();'>" + json.name + "</a></li>";
    $("#selected-file-list-group").append(li);
    $('[data-toggle="tooltip"]').tooltip();
}

function selectServerFile() {
    const json = app.serverFiles[$(event.srcElement).parent().attr("data-key")];
    if (json.isDirectory) {
...
function removeSelectedServerFile() {
    const liEle = $(event.srcElement).parent();
    const key = $(liEle).attr("data-key");
...
function updateAuth(url, down, uplo, dele, upda, visi, key) {
    const auth = toOneOrZero(down) + "," + toOneOrZero(uplo) + "," + toOneOrZero(dele) + "," + toOneOrZero(upda) + "," + toOneOrZero(visi);
    layer.load(1);
    $.ajax({
        url: url, type: "PUT", data: {auth: auth}, success: function (data) {
            layer.closeAll();
            const result = data.indexOf("success") > 0;
        ...
        layer.confirm('是否确定删除选中的所有行', {
            ...
            $.ajax({
                url: url + ids, type: "DELETE", success: function (data) {
                    ...
                    if (json.status === "success") {
                        selectedRows.forEach(function (tr) {
                            const key = $(tr).children(index).attr("data-key");
...
$(document).ready(function () {
    $("#auth-add-button").click(function () {
        const fileList = $("#auth-file-list-group").find("a");
        let files = "";
        for (let i = 0; i < fileList.length; i++) {
            files += $(fileList[i]).attr("data-key") + ",";
        }
        const userList = $("#auth-user-list-group").find("a");
        let users = "";
        ...
        if (isEmpty(files) || isEmpty(users)) {
            ...
            const down = $("#auth-downloadable-new").val();
            const dele = $("#auth-deletable-new").val();
            const upda = $("#auth-updatable-new").val();
            const visi = $("#auth-visible-new").val();
            const auths = toOneOrZero(down) + ",1," + toOneOrZero(dele) + "," + toOneOrZero(upda) + "," + toOneOrZero(visi);
    ...
    $("#user-auth-update-button").click(function () {
        const key = $(selectedRows[rowIndex]).children(".user-index").attr("data-key");
        const permission = $("#userAuthModal").find(".user-permission").val();
        layer.load(1);
        $.ajax({
    ...
    $("#user-password-update-button").click(function () {
        const password = $("#userPasswordModal").find(".user-password").val();
        if (isEmpty(password)) {
            alerts("密码不能为空");
        } else {
            const user = app.users[$(selectedRows[rowIndex]).children(".user-index").attr("data-key")];
    ...
    $("#auth-update-button").click(function () {
        const down = $("#auth-downloadable").val();
        const dele = $("#auth-deletable").val();
        const upda = $("#auth-updatable").val();
        const visi = $("#auth-visible").val();
        const key = $(selectedRows[rowIndex]).children(".auth-index").attr("data-key");
        const id = app.auths[key].id;
        updateAuth("/auth/" + id, down, 1, dele, upda, visi, key);
    });
    $("#user-file-auth-update-button").click(function () {
        const key = $(selectedRows[rowIndex]).children(".user-index").attr("data-key");
        const parent = $("#userFileAuthModal");
        const down = $(parent).find(".user-downloadable").val();
        const uplo = $(parent).find(".user-uploadable").val();
        const dele = $(parent).find(".user-deletable").val();
        const upda = $(parent).find(".user-updatable").val();
        updateAuth("/user/" + app.users[key].id + "/auth", down, uplo, dele, upda, 1, key);
    });
    $("#file-auth-update-button").click(function () {
        const down = $("#file-downloadable-auth").val();
        const dele = $("#file-deletable-auth").val();
        const upda = $("#file-updatable-auth").val();
        const visi = $("#file-visible-auth").val();
        const id = app.files[$(selectedRows[rowIndex]).children(".file-index").attr("data-key")].id;
        //无须传递KEY
    ...
    $(".server-path-return").click(function () {
        const len = serverFileSearchHistory.length;
        if (len > 1) {
    ...
    $("#file-modify-button").click(function () {
        const localUrl = $("#new-file-local-url").val();
        const visitUrl = $("#new-file-visit-url").val();
        if (isEmpty(localUrl) && isEmpty(visitUrl)) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    layer.closeAll();
                    const json = JSON.parse(data);
                    const key = $(selectedRows[rowIndex]).children(".file-index").attr("data-key");
