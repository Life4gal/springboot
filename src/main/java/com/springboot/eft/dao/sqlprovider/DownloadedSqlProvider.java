...
public class DownloadedSqlProvider {
	...
	public String getDownloadBy(@Param("userId") int userId, @Param("fileId") long fileId, @Param("fileName") String
		...
		String sql = new SQL() {{
			...
			ORDER_BY("d." + EftApplication.settings.getStringUseEval(ConfigConstants.DOWNLOAD_ORDER_BY_OF_SETTINGS));
		}}.toString();
		int size = EftApplication.settings.getIntegerUseEval(ConfigConstants.DOWNLOAD_PAGE_SIZE_OF_SETTINGS);
