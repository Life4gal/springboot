...
public class AuthSqlProvider {
	...
	public String getAuthBy(@Param("id") long id, @Param("userId") int userId, @Param("fileId") long fileId, @Param
		...
		String sql = new SQL() {{
			...
			ORDER_BY("a." + EftApplication.settings.getStringUseEval(ConfigConstants.AUTH_ORDER_BY_OF_SETTINGS));
		}}.toString();
		int size = EftApplication.settings.getIntegerUseEval(ConfigConstants.AUTH_PAGE_SIZE_OF_SETTINGS);
