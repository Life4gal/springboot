...
public class Utils {
	...
	public static <T> T loadJsonToBean(String jsonPath, Class<T> clazz) throws IOException {
		return loadJsonToBean(jsonPath, ValueConstants.NULL_STRING, clazz);
	}
	...
	public static String extractDigit(String string) {
		...
		if (HAS_DIGIT_PATTERN.matcher(string).matches()) {
			...
			if (res.indexOf(ValueConstants.DOT_SIGN) == 0) {
				res.insert(0, "0");
			} else if (res.indexOf(ValueConstants.NEGATIVE_DOT_SIGN) == 0) {
	...
	public static String getCurrentWorkDir() {
		return Paths.get(ValueConstants.DOT_SIGN).toAbsolutePath().normalize().toString();
	}
	...
	public static int getMaxValue(int... nums) {
		...
		for (int i = 0; i < last; i += ValueConstants.TWO_INT) {
