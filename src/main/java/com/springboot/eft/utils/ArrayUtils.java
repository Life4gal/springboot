...
public class ArrayUtils {
	...
	public static void heapSort(int[] arrays) {
		// 将待排序的序列构建成一个大顶堆
		for (int i = arrays.length / ValueConstants.TWO_INT; i >= 0; i--) {
	...
	private static void heapAdjust(int[] arrays, int i, int n) {
		int child;
		int father;
		for (father = arrays[i]; ValueConstants.TWO_INT * i + 1 < n; i = child) {
			child = 2 * i + 1;
	...
	private static void mergeSort(int[] arrays, int low, int mid, int high) {
		...
		System.arraycopy(temp, ValueConstants.ZERO_INT, arrays, low, temp.length);
	}
	...
	public static void shellSort(int[] arrays) {
		int h = 1;
		while (h <= arrays.length / ValueConstants.THREE_INT) {
			h = h * 3 + 1;
		}
		while (h > 0) {
