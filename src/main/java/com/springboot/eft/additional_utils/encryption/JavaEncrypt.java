package com.springboot.eft.additional_utils.encryption;
	...
	private static String cryptDES(String string, int opmode, String key) throws NoSuchAlgorithmException,
		...
		if (isInit == 1 || initDES(key) || initDES(key)) {
			if (DES_CRYPT_WAY.equals(key)) {
				...
				result = new String(cipher.doFinal(string.getBytes(StandardCharsets.ISO_8859_1)), StandardCharsets.ISO_8859_1);
			} else if (DES3_CRYPT_WAY.equals(key)) {
				cipher3.init(opmode, secretKey3);
				result = new String(cipher3.doFinal(string.getBytes(StandardCharsets.ISO_8859_1)), StandardCharsets.ISO_8859_1);
			} else if (AES_CRYPT_WAY.equals(key)) {
				aes.init(opmode, aesKey);
				result = new String(aes.doFinal(string.getBytes(StandardCharsets.ISO_8859_1)), StandardCharsets.ISO_8859_1);
			} else {
				rsa.init(opmode, opmode == Cipher.ENCRYPT_MODE ? rsaKey.getPublic() : rsaKey.getPrivate());
				result = new String(rsa.doFinal(string.getBytes(StandardCharsets.ISO_8859_1)), StandardCharsets.ISO_8859_1);
			}
		}
		return result;
	}
