...
public class Downloader {
	 ...
	 * 默认下载目录
	 */
	private static String storageFolder = ValueConstants.USER_HOME + ValueConstants.SEPARATOR + "util";
	...
	public static boolean download(String downloadURL) {
		...
		try {
			String fileName = checkPath(storageFolder + ValueConstants.SEPARATOR + Formatter.getFileName(downloadURL));
			...
			if (conn.getResponseCode() != ValueConstants.RESPONSE_OK || conn.getContentLength() <= 0) {
				return false;
			}
