...
import com.springboot.eft.additional_utils.annotation.FieldChecking;
import com.springboot.eft.additional_utils.constant.ValueConstants;
import com.springboot.eft.additional_utils.interfaces.IChecker;
import com.springboot.eft.additional_utils.model.CheckResult;
import com.springboot.eft.additional_utils.model.ResultObject;
...
public class Checker {
	...
	public static <T> CheckResult<T> checkBean(Object bean) {
		return checkBean(bean, new HashMap<>(ValueConstants.TWO_INT));
	}
		...
		for (Field field : fields) {
			...
			if (isNotNull(checking)) {
				...
				try {
					...
					object.status = ValueConstants.ERROR_EN;
				...
				} else if (expression.startsWith(ValueConstants.COLON)) {
					// 正则匹配
					result.passed = isNotNull(value);
					if (result.passed) {
	...
	public static String checkNull(String value) {
		return checkNull(value, ValueConstants.EMPTY_STRING);
	}
