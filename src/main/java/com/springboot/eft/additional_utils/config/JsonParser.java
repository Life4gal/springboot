package com.springboot.eft.additional_utils.config;
...
public class JsonParser {
	 ...
	 * 用来存储已经解析过Json对象
	 */
	private final HashMap<String, JSONObject> jsonStore = new HashMap<>(ValueConstants.SIXTEEN_INT);
	/**
	 * 最大缓存大小
	 */
	...
	private String checkPath(String key) {
		if (!key.startsWith(ValueConstants.DOLLAR_SIGN)) {
			if (!key.startsWith(ValueConstants.DOT_SIGN)) {
				key = "." + key;
			}
			key = "$" + key;
		}
		if (key.endsWith(ValueConstants.DOT_SIGN)) {
	...
	private String pathToKey(String path) {
		if (path.startsWith(ValueConstants.DOLLAR_SIGN)) {
			path = path.substring(1);
		}
		if (!path.startsWith(ValueConstants.DOT_SIGN)) {
			path = ValueConstants.DOT_SIGN + path;
		}
		if (!path.endsWith(ValueConstants.DOT_SIGN)) {
