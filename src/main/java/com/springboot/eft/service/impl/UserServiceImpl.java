...
import com.springboot.eft.additional_utils.DateUtils;
...
import com.springboot.eft.dao.UserDAO;
...
public class UserServiceImpl implements IUserService {
	...
	public boolean updatePermission(int id, int permission) {
		return userDAO.updatePermission(id, Math.min(permission, 2));
	}
	...
	public User login(String loginName, String password, String token, HttpServletResponse response) {
		boolean allowLogin = settings.getBooleanUseEval(ConfigConstants.ALLOW_LOGIN_OF_SETTINGS);
		User user = null;
		if (allowLogin) {
			if (Checker.isNotEmpty(token) && EftApplication.tokens.containsKey(token)) {
				user = userDAO.getUserById(EftApplication.tokens.get(token));
				if (Checker.isNotNull(response)) {
					Cookie cookie = new Cookie(ValueConstants.TOKEN_STRING, TokenConfig.generateToken(token, user.getId
							()));
	...
	public boolean register(String username, String email, String password) {
		boolean allowRegister = settings.getBooleanUseEval(ConfigConstants.ALLOW_REGISTER_OF_SETTINGS);
		if (allowRegister) {
			boolean isValid = Checker.isEmail(email) && checkPassword(password) && Pattern.compile(settings
					.getStringUseEval(ConfigConstants.USERNAME_PATTERN_OF_SETTINGS)).matcher(username).matches();
			if (isValid) {
				User user = new User(username, ValueConstants.EMPTY_STRING, email, password);
				int[] auth = SettingConfig.getAuth(ConfigConstants.USER_DEFAULT_AUTH_OF_SETTING);
	...
	public boolean checkPassword(String password) {
		int min = settings.getIntegerUseEval(ConfigConstants.PASSWORD_MIN_LENGTH_OF_SETTINGS);
		int max = settings.getIntegerUseEval(ConfigConstants.PASSWORD_MAX_LENGTH_OF_SETTINGS);
