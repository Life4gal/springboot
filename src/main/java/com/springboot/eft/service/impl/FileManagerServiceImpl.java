...
import com.springboot.eft.service.IFileManagerService;
...
public class FileManagerServiceImpl implements IFileManagerService {
	...
	public void multiDownload(HttpServletResponse response, String[] items, String destFile) throws IOException {
		File zip = ZipUtil.zip(new File(ValueConstants.USER_DESKTOP + File.separator + destFile), ValueConstants.FALSE,
	...
	public JSONObject upload(String destination, MultipartFile... files) {
		...
		if (Checker.isNotEmpty(files)) {
			if (Checker.isWindows() && destination.length() < ValueConstants.TWO_INT) {
				destination = "C:";
			}
			for (MultipartFile file : files) {
				if (Checker.isNotNull(file) && !file.isEmpty()) {
					try {
						...
						return getBasicResponse(ValueConstants.FALSE);
					}
				}
			}
		}
		return getBasicResponse(ValueConstants.TRUE);
	}
	...
	public JSONObject compress(JSONObject object) {
		...
		File zip = ZipUtil.zip(new File(dest + File.separator + name), ValueConstants.FALSE, files);
	...
	public JSONObject move(JSONObject object) {
		...
		for (Object file : array) {
			try {
				FileExecutor.moveToDirectory(new File(file.toString()), new File(dest), ValueConstants.TRUE);
			} catch (IOException e) {
				logger.error(e.getMessage());
				return getBasicResponse(ValueConstants.FALSE);
			}
	...
	public JSONArray list(JSONObject object) {
		...
		if (Checker.isWindows()) {
			if (Checker.isNotEmpty(path) && path.startsWith(ValueConstants.SPLASH_STRING)) {
