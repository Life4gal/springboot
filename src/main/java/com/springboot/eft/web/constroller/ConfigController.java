...
public class ConfigController {
	...
	public String updateConfig(String config) {
		...
		if (user.getPermission() > ValueConstants.TWO_INT) {
			EftApplication.settings.setJsonObject(config);
			//打包成jar之后无法修改config.json文件
			try {
				FileExecutor.saveFile(NetUtils.urlToString(EftApplication.class.getResource(DefaultValues
						.SETTING_PATH)), EftApplication.settings.toString());
			} catch (IOException e) {
	...
	public String getAll() {
		User user = (User) request.getSession().getAttribute(ValueConstants.USER_STRING);
		if (user.getPermission() > ValueConstants.TWO_INT) {
			return EftApplication.settings.toString();
		} else {
			return "{\"message\":\"permission denied\"}";
		}
	}
