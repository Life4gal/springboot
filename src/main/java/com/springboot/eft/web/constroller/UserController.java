...
import com.springboot.eft.additional_utils.encryption.JavaEncrypt;
...
public class UserController {
	...
	public String updatePermission(@PathVariable("id") int id, @PathVariable("permission") int permission) {
		...
		if (user.getPermission() < ValueConstants.THREE_INT && permission > 1) {
	...
	public String getUser(String user, int offset) {
		User u = (User) request.getSession().getAttribute(ValueConstants.USER_STRING);
	...
	public String updateBasicInfo(String avatar, String realName, String email, String code) {
		...
		boolean emilVerify = EftApplication.settings.getBooleanUseEval(ConfigConstants.EMAIL_VERIFY_OF_SETTINGS);
	...
	public String getInfo() {
		...
		object.remove(ValueConstants.ID_STRING);
		object.remove(ValueConstants.PASSWORD_STRING);
	...
	public String login(String username, String password, boolean auto, String token) {
		//使用密码登录
		User user = userService.login(username, password, ValueConstants.NULL_STRING, ValueConstants.NULL_RESPONSE);
